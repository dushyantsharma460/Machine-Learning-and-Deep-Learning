# I am using train test split
# 80% data -> for training   20% data -> for testing 

# You can also download the data from these following website
# 1. openml.org
# 2. uci ml repository (Popular dataset -> Iris)
# 3. kaggle
# 4. tensorflow.org
...
# Think of np.random.seed() like a lock code 🔑 for randomness.
# Normally, random numbers are different every time.
# If you set a seed number, you tell the
# computer: 👉 "Always start from the same place when picking


# # Fixed seed
# np.random.seed(42)
# print(np.random.randint(1, 10, 5))   # Always same
# Not change every run

# # No seed (different each run)
# np.random.seed()  
# print(np.random.randint(1, 10, 5))   # Changes every run
